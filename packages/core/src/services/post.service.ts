...
export class PostService {
    ...
    async resolveHomeData(tenantId: string, config: SiteConfig): Promise<HomeData> {
        ...
        const { results: posts } = await this.db
            .prepare(
                "SELECT * FROM posts WHERE tenant_id = ? ORDER BY published_at DESC, last_edited_time DESC"
            )
            .bind(tenantId)
            .all();
    ...
    async getPostsSimple(tenantId: string) {
        const { results } = await this.db
            .prepare("SELECT id, title, slug FROM posts WHERE tenant_id = ? ORDER BY title ASC")
            .bind(tenantId)
            .all();
        return results || [];
    }
